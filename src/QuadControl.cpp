...
V3F QuadControl::RollPitchControl(V3F accelCmd, Quaternion<float> attitude, float collThrustCmd)
{
  ...
  float target_R13, target_R23, b_dot_x, b_dot_y;
  ...
  b_dot_x = kpBank * (target_R13 - R(0, 2));
  b_dot_y = kpBank * (target_R23 - R(1, 2));
...
float QuadControl::YawControl(float yawCmd, float yaw)
{
  ...
  float yaw_mod, yawCmd_mod, yaw_error;
  float pi = 3.14159265;
  yaw_mod = fmodf(yaw, 2 * pi);
  yawCmd_mod = fmodf(yawCmd, 2 * pi);
  yaw_error = yawCmd_mod - yaw_mod;
  
  //yaw_error = atan2f(cos(yaw_error), sin(yaw_error));
